<template>
    <div class="insert-paper-content">
                    ...
                    <div class="paper-selection">
                        ...
                        <select id="paperSelect" v-model="selectedPaperForAnswers" class="paper-select">
                            ...
                            <option v-for="paper in filteredPapers" :key="paper.paper_name" :value="paper.paper_name">
                                {{ paper.has_answer_key === false ? '! ' : '' }}{{ paper.paper_name }} ({{ paper.question_count }} questions{{ paper.has_answer_key === false ? ', no answers' : '' }})
                            </option>
                        </select>
                        <small class="warning-legend" v-if="filteredPapers.some(p => !p.has_answer_key)">
                            ! = Paper has no answer keys
                        </small>
                    </div>
...
import API_BASE_URL, { authFetch, getToken } from '../config/api.js';
        ...
        preprocessAnswersForDatabase(extractedAnswers) {
            ...
            for (const answer of extractedAnswers) {
                ...
                if (!mainQuestionMatch) {
                    console.warn(`Could not extract main question number from: ${questionNum}`);
                    continue;
                }
